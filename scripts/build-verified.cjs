...
(async function main(){
  ...
  let child;
  if (process.platform === 'win32') {
    const vinextCmd = path.join(projectRoot, 'node_modules', '.bin', 'vinext.cmd');
    if (!fs.existsSync(vinextCmd)){
      console.error('vinext.cmd is unavailable. Run npm run install:ci and wait for it to finish before building.');
      process.exit(69);
    }
    child = spawn('cmd.exe', ['/c', vinextCmd, 'build'], {env, stdio: 'inherit'});
  } else {
    // use local bin path on unix
    const vinextBin = path.join(projectRoot, 'node_modules', '.bin', 'vinext');
    child = spawn(vinextBin, ['build'], {env, stdio: 'inherit'});
  }
